...
echo"  <title> Case Documents - ".site_name()."</title>";
...
<div class="table-responsive">
		<table class="table table-striped table-bordered table-hover">
			<thead>
					<tr>
                                                <th><div class="label label-success">  Case #</div></th>
                                                
 <th><div class="label label-success">  Link Document</div></th>
...
$statement = "cases WHERE case_attachment>'0'$addsel"; 



$select_case = $pdo->prepare("SELECT * FROM cases WHERE case_attachment>'0'$addsel ORDER BY case_date DESC LIMIT $startpoint,$per_page");
}
elseif(preg_match('/is_highcourt/i', $is_usertype))
{
$statement = "cases WHERE case_attachment>'0' AND case_to='$user'$addsel"; 

	$select_case = $pdo->prepare("SELECT * FROM cases WHERE case_attachment>'0' AND case_to='$user'$addsel ORDER BY case_date DESC LIMIT $startpoint,$per_page");
  }
elseif(preg_match('/is_court/i', $is_usertype))
{
$statement = "cases WHERE case_attachment>'0' AND case_posted='$user'$addsel"; 

	$select_case = $pdo->prepare("SELECT * FROM cases WHERE case_attachment>'0' AND case_posted='$user'$addsel ORDER BY case_date DESC LIMIT $startpoint,$per_page");
  }
else
{
$statement = "cases WHERE case_from='$user'$addsel"; 

	$select_case = $pdo->prepare("SELECT * FROM cases WHERE case_attachment>'0' AND case_from='$user'$addsel ORDER BY case_date DESC LIMIT $startpoint,$per_page");
}
...
$colorstatus="success";
			...
			?>
                        ...
                        <tr>
                                                ...
                                                <td>
<a href="<?php echo $site_url; ?>/documents/<?php echo $case_attachment; ?>"><?php echo $case_name; ?></a></td>
                                                


<td><a href="<?php echo $site_url; ?>/documents/<?php echo $case_attachment; ?>"><button class='btn btn-info'>Export</button>
</a>
...
<a href="<?php echo $site_url; ?>/manage/edit.php?id=<?php echo $case_id; ?>"><button class='btn-lg btn-warning'>Mark as draft</button>
</a>
<hr/>
<?php }
}
...
<form method="POST" action="edit.php?id=<?php echo $case_id; ?>">
...
<a href="javascript:;" data-toggle="collapse" data-target="#edit-it<?php echo $case_id; ?>"><button class='btn-lg btn-info'>Edit Case</button>
    ...
    <div id="edit-it<?php echo $case_id; ?>" class="collapse">
		...
		<div style="height:10px;"></div>
					<div class="row">
						<div class="col-lg-2">
							<label style="position:relative; top:7px;">Case Document: (<?php echo $case_attachment; ?>)</label>
						</div>
						<div class="col-lg-10">
...
?>
	...
	<?php //include('addorder_modal.php'); 
include "modal.php";
?>
</div>
  <!-- jQuery -->
